...
var abc = ['laydate_box', 'laydate_void', 'laydate_click', 'LayDateSkin', 'skins/', '/laydate.css'];
...
win.laydate = function(options){
    options = options || {};
    try{
        abc.event = win.event ? win.event : laydate.caller.arguments[0];
    } catch(e){};
    Dates.run(options);
    return laydate;
};
...
Dates.use = function(lib, id){
    ...
    link.type = 'text/css';
    link.rel = 'stylesheet';
    link.href = Dates.getPath + lib + abc[5];
    id && (link.id = id);
...
Dates.run = function(options){
    var S = Dates.query, elem, devt, even = abc.event, target;
    try {
    ...
    abc.elemv = /textarea|input/.test(elem.tagName.toLocaleLowerCase()) ? 'value' : 'innerHTML';
    if (config.init) elem[abc.elemv] = laydate.now(null, options.format || config.format)
...
Dates.check = function(){
    ...
    var exp = new RegExp(reg), value = Dates.elem[abc.elemv];
    var arr = value.match(/\d+/g) || [], isvoid = Dates.checkVoid(arr[0], arr[1], arr[2]);
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            Dates.elem[abc.elemv] = '';
            ...
            isvoid.value = Dates.elem[abc.elemv].match(exp).join();
            ...
            } else if(isvoid.value !== Dates.elem[abc.elemv]){
                Dates.elem[abc.elemv] = isvoid.value;
            }
        }
    }
};
...
Dates.viewDate = function(Y, M, D){
    ...
    //渲染日
    Dates.each(abc.tds, function(i, elem){
    ...
    Dates.valid = !Dates.hasClass(log.thisDay, abc[1]);
    Dates.ymd = log.ymd;

    //锁定年月
    abc.year.value = Dates.ymd[0] + '年';
    abc.month.value = Dates.digit(Dates.ymd[1] + 1) + '月';

    //定位月
    Dates.each(abc.mms, function(i, elem){
        var getCheck = Dates.checkVoid(Dates.ymd[0], (elem.getAttribute('m')|0) + 1);
        if(getCheck[0] === 'y' || getCheck[0] === 'm'){
            Dates.addClass(elem, abc[1]);
        } else {
            Dates.removeClass(elem, abc[1]);
        }
        Dates.removeClass(elem, abc[2]);
        getCheck = null
    });
    Dates.addClass(abc.mms[Dates.ymd[1]], abc[2]);
    ...
    //确定按钮状态
    Dates[Dates.valid ? 'removeClass' : 'addClass'](abc.ok, abc[1]);
};
...
Dates.viewYears = function(YY){
    ...
    Dates.each(new Array(14), function(i){
        if(i === 7) {
            str += '<li '+ (parseInt(abc.year.value) === YY ? 'class="'+ abc[2] +'"' : '') +' y="'+ YY +'">'+ YY +'年</li>';
        } else {
...
Dates.initDate = function(){
    var S = Dates.query, log = {}, De = new Date();
    var ymd = Dates.elem[abc.elemv].match(/\d+/g) || [];
    if(ymd.length < 3){
...
Dates.iswrite = function(){
    ...
    Dates.shde(abc.oclear, !('isclear' in Dates.options ? Dates.options.isclear : 1));
    Dates.shde(abc.otoday, !('istoday' in Dates.options ? Dates.options.istoday : 1));
    Dates.shde(abc.ok, !('issure' in Dates.options ? Dates.options.issure : 1));
};
...
Dates.reshow = function(){
    Dates.each(Dates.query('#'+ abc[0] +' .laydate_show'), function(i, elem){
...
Dates.close = function(){
    Dates.reshow();
    Dates.shde(Dates.query('#'+ abc[0]), 1);
    Dates.elem = null;
};
...
Dates.events = function(){
    ...
    abc.tds = S('#laydate_table td');
    abc.mms = S('#laydate_ms span');
    abc.year = S('#laydate_y');
    abc.month = S('#laydate_m');
    ...
    Dates.each(S(log.box + ' .laydate_ym'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            ...
            if(!i){
                log.YY = parseInt(abc.year.value);
    ...
    //清空
    abc.oclear = S('#laydate_clear');
    Dates.on(abc.oclear, 'click', function(){
        Dates.elem[abc.elemv] = '';
        Dates.close();
    });

    //今天
    abc.otoday = S('#laydate_today');
    Dates.on(abc.otoday, 'click', function(){
        var now = new Date();
    ...
    //确认
    abc.ok = S('#laydate_ok');
    Dates.on(abc.ok, 'click', function(){
        if(Dates.valid){
...
Dates.init = (function(){
    Dates.use('need');
    Dates.use(abc[4] + config.defSkin, abc[3]);
    Dates.skinLink = Dates.query('#'+abc[3]);
}());
...
laydate.skin = function(lib){
    Dates.skinLink.href = Dates.getPath + abc[4] + lib + abc[5];
};
