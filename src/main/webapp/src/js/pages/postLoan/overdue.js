...
var dtTable;
function initOverdueList() {
    dtTable = $("#overdueTable").DataTable({
    serverSide: true,//服务端分页
    searching: false,//显示默认搜索框
    ordering: false,//开启排序
    autoWidth: true,//自适应宽度
    "ajax": {
        "url":'/post_loan/query_overdue_list',
        "type":'POST',
        "data": function (d) {
    ...
    columnDefs:[{
        ...
        targets:6,
        render:function(data,type,row,meta){
            return row.dueDate == null || row.dueDate == '--'? '--' : moment(row.dueDate).format('YYYY-MM-DD');
        }
    },{
        targets:7,
        ...
        className:'right aligned',
        render:function(data,type,row,meta){
            return row.otherFee == null ? '--' : accounting.formatNumber(row.otherFee,2,",");
        }
    },{
        targets:10,
        render:function(data,type,row,meta){
            return row.dueDays == null ? '--' : row.dueDays;
        }
    },{
        targets:11,
        render:function(data,type,row,meta){
...
function fetchById(repayId,id) {
    window.open("/business_apply/detail?id="+id+"&repayId="+repayId+"&type=OVERDUE"+"&tab=postLoan"+"&comeFrom="+window.location.pathname)
}
