...
var minSeasons = parseInt($("#stepData").data("min_season"));
var maxSeasons = parseInt($("#stepData").data("max_season"));
...
function getBinChoice(type,value) {
    ...
    }else if(termType == 'SEASONS'){
        code = $("#stepData").data("interest_season_type")+"";
    }else{
...
function selectForPaymethod(){
    ...
    if (_termTypeVal == 'DAYS') {
        selectRepayethod('INTEREST_MONTHS/EQUAL_INSTALLMENT/INTEREST_SEASONS');
        $("#minInterestAmountDiv").removeClass("ks-hidden");
        $("#calculateMethodAboutDay").removeClass("ks-hidden");
    } else if (_termTypeVal == 'MOTHS') {
        selectRepayethod('INTEREST_DAYS/INTEREST_SEASONS');
    ...
    fields: {
        'term': {
            ...
            rules: [{
                ...
                type: 'validateTerm4',
                prompt: '借款期限为整数，且在' + minSeasons + '-' + maxSeasons + '季'
            }]
        },
        'repayMethod':{
            rules: [{
        ...
        'interestAmount': {
            ...
            rules: [{
                ...
                type: 'validateInterestAmount[SEASONS]',
                prompt: '借款利息（金额计息）不正确(不小于' + parseFloat($("#stepData").data("min_interest_season_amount")) + '元)'
            },{
        ...
        'interestRate': {
            ...
            rules: [{
                ...
                type: 'validateInterestRate[SEASONS]',
                prompt: '借款利息（比例计息）不正确(不小于' + parseFloat($("#stepData").data("min_interest_season_rate")) + '%)'
            },{
...
drop_list = {
    ...
    SEASONS:{FIX_AMOUNT:"金额计息(元)/季",FIX_RATE:"比例计息(%)/季"}
};
//最小值判断
termList = {
    ...
    SEASONS: {amount:'min_interest_season_amount',rate:'min_interest_season_rate'}
};
...
$("#step2Form select[name='termType']").change(function() {
    ...
    if(termType == "MOTHS" || termType == "SEASONS"){
...
$.fn.form.settings.rules.validateTerm4=function(value){
    var termType = $("#step2Form select[name='termType']").val();
    if(termType == "SEASONS") {
        var minSeasons = parseInt($("#stepData").data("min_season"));
        var maxSeasons = parseInt($("#stepData").data("max_season"));
        if(isNaN(value) || parseInt(value) < minSeasons || parseInt(value) > maxSeasons||!isInteger(value)) {
            return false;
        } else {
            return true;
        }
...
var day_amount = $('#prdDate').data('day_amount'),
    ...
    sea_amount = $('#prdDate').data('sea_amount'),
    sea_rate = $('#prdDate').data('sea_rate');
var loanLimitType = $('#prdDate').data('loan_limittype');
var amount = $('#step2Form input[name="interestAmount"]'),
    rate = $('#step2Form input[name="interestRate"]');
...
$("#step2Form select[name='repayMethod']").change(function() {
    ...
    if(repayMethod == "INTEREST_MONTHS") {
        ...
        $("#step2Form select[name='repayDateType']").addClass('disabled',true);
    } else {
...
/**
 * Created by yangzb01 on 2017-07-20.
 */
