...
$("#channelSearch").search({
    ...
    onSelect: function (data) {
        ...
        $("#step1Form input[name='saleId'],#step1Form input[name='engagedSaleId']").val(data.id);
...
// 请求更新借款人
function ajaxUpdateLoanBorrower(callback) {
    $(document).api({
        on: "now",
        action: "update loan borrower",
        method: "post",
        beforeSend: function (settings) {
...
// 请求更新账户信息
function ajaxUpdateLoanAccount(callback) {
    $(document).api({
        on: "now",
        action: "update loan account",
        method: "post",
        beforeSend: function (settings) {
            var loan_id = $("#stepData").data("loan_id");
            settings.data["id"] = loan_id;
            ...
            $("#main_banks .relative").each(function () {
                ...
                accounts.push({
                    ...
                    'account': account,
                    'amount': $item.find("input[name='account.amount']").val(),
...
$(document).on("click", "#btn-submit", function () {
    ...
    if (flag) {
        if($('#templateType').val()&&$('#templateType').val().indexOf('hongbenAdd') > -1){
            $('#linkStep3').removeClass('completed'); //hongben
        }
