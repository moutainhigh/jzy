/**
 * Created by yangzb01 on 2017-07-19.
 */
/**
...
var channelType = $("#stepData").data("channel_type");
var certifType;
...
function initSlectType(obj){
    if(obj){
        ...
        var val = $('#step1Form select[name="channelType"]').val();
    }
    if(val == '0'){
    ...
    channel:function(){
        $('#managerChannelSearch').search({
            ...
            onSelect: function (data) {
                ...
                $("#step1Form input[name='channelId']").val(data.id);
                if($('select[name="channelType"]').val() == '1'){
                    $("#step1Form input[name='saleId']").val(data.managerId);
                }
    ...
    partner:function(){
        $('#partnerSearch').search({
            ...
            onSelect: function (data) {
                ...
                $("#step1Form input[name='channelId'],#step1Form input[name='engagedSaleId']").val(data.id);
                $("#step1Form input[name='engagedSaleName']").val(data.name);
            }
        });
    },
    ...
    manager:function(){
        $("#channelSearch").search({
            ...
            onSelect: function (data) {
                ...
                $this.find('.js_search').addClass('close');
                $this.find('.js_search').css('pointer-events','auto');
                $("#step1Form input[name='saleId']").val(data.id);
            }
        });
    },
    ...
    engagedManager:function(){
        $("#engagedChannelSearch").search({
            ...
            onSelect: function (data) {
                ...
                $this.find('.js_search').addClass('close');
                $this.find('.js_search').css('pointer-events','auto');
                $("#step1Form input[name='engagedSaleId']").val(data.id);
            }
        });
    }
};
...
var borrower = {
        ...
        $("#btn-eidtBorrower").click(function () {
            ...
            $('#form-editBorrower').form(validateOptions).api({
                ...
                onSuccess: function (response) {
                    ...
                    $("#borrower-" + borrowerId).find(".borrower-companyProfiles").html(response.data.companyProfiles);
                    $("#companyProfiles").val(response.data.companyProfiles)
    ...
    edit:function(){
        $(document).on("click", ".btn-borrowerEdit", function () {
            ...
            $("#modal-editBorrower").find("textarea[name='companyProfiles']").val($("#borrower-" + borrowerId).find('.borrower-companyProfiles').html());
...
function update_bl(){
    $(document).on("click", "#btn-submit", function () {
        ...
        var channelType = $('#step1Form select[name="channelType"]').val();
        var partnerName = $('#step1Form input[name="partnerName"]').val();
        if(channelType == 0 &&  partnerName==''){
            $("#step1Form input[name='channelId']").val("");
        }
        var flag = true;
        ...
        $("#step1Form").form({
            fields: {
                ...
                'companyProfiles': {
                    ...
                    rules: [{
                        type: 'empty',
                        prompt: '请填写主借款人的公司基本情况，最大500个字符'
                    }]
                },
                onSuccess: function (e, fidlds) {
                    e.preventDefault();
        ...
        if (flag) {
            $("#step1Form").api({
                ...
                beforeSend: function (settings) {
                    ...
                    if($('#step1Form select[name="channelType"]').val() == '0'){
                        settings.data["channelSearchName"] = $('#step1Form').find("input[name='partnerName']").val();
                    }
                ...
                onSuccess: function (response) {
                    $("#linkStep1").addClass("completed");
                    $("#step2Form input[name='amount']").val(accounting.toFixed(response.data.amount, 2));
                    goStep("Step2");
...
function init(){
    $(document).ready(function(){
        var selectList = ['org.managerName','partnerName','channelSearchName','engagedSaleName'];
        for(i=0; i<selectList.length;i++){
            var bindObj = $('input[name="'+selectList[i]+'"]').siblings('.js_search');
            if($('input[name="'+selectList[i]+'"]').val() !=''){
                $('input[name="'+selectList[i]+'"]').attr('readonly',true);
                bindObj.addClass('close').removeClass('search');
                bindObj.css('pointer-events','auto');
            }else{
                $('input[name="'+selectList[i]+'"]').attr('readonly',false);
                bindObj.removeClass('close').addClass('search');
                bindObj.css('pointer-events','none');
            }
        }
    });


    $('#step1Form select[name="channelType"]').change(function(){
        var val = $(this).val();
        initSlectType(val);
    });
    ...
    $(document).on('click','.js_search',function(){
        ...
        $(this).addClass('search').removeClass('close');
    });
}
init();
