/**
 * Created by yangzb01 on 2017-06-21.
 */
...
function queryLoanFee(repayMethod){
    ...
    $(document).api({
        ...
        onSuccess: function (data) {
            ...
            //计算实际放款金额
            /*
            * 节点为LOAN_NODE，放款类型为LOAN_AMOUNT_RATE，LOAN_REQUEST_INPUT，FIXED_AMOUNT的计算
            * */
            var allAmount = parseFloat($('#step2Form').find('input[name="amount"]').val());
            var rateAmount = 0,
                elseAmount =0;
                requestId =[];
            for(i in _data){
                var val = _data[i];
                if(val.chargeNode =='LOAN_NODE'){
                    if(val.chargeType =='LOAN_AMOUNT_RATE'){
                        rateAmount =add(rateAmount, mul(allAmount,(val.feeRate/100)));
                    }else if(val.chargeType == 'LOAN_REQUEST_INPUT' || val.chargeType == 'FIXED_AMOUNT'){
                        elseAmount =add(elseAmount,val.feeAmount)
                    }

                    if(val.chargeType == 'LOAN_REQUEST_INPUT'){
                        requestId.push(val.id);
                    }
                }
            }
            var else_allAmount = add(rateAmount,elseAmount);
            $('#actualLoanAmount').val(sub(allAmount,else_allAmount).toFixed(2));
...
//手动改变feeAmount计算实际放款金额
function setActualAmount(){
    $(document).on('propertychange input','#step2Form .feeAmount',function(){
        var id = ""+$(this).attr('name').split('_')[1]+"";
        for(i in requestId){
            if(requestId[i] == id){
                var val = $(this).val() ==''? 0:$(this).val();
                var $thisVal = parseFloat(val);
                var actVal = parseFloat($('#actualLoanAmount').val());
                var defaultValue = sub($thisVal,parseFloat($(this).attr('value')));
                $('#actualLoanAmount').val(sub(actVal,defaultValue).toFixed(2));
