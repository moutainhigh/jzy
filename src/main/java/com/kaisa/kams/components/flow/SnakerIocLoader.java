...
//    	开始构建sanker的配置对象
        ...
        if (paths != null) {
            for (Object path : paths) {
                for(NutResource re : Scans.me().scan(String.valueOf(path))) {
                    ...
                    List<WfProcess> list = ctx.find(DBAccess.class).queryList(WfProcess.class, querySql, model.getName());
                    if (!list.isEmpty()) {
                        WfProcess p = list.get(0);
                        byte[] cnt = p.getContent();
    ...
    @Override
    public IocObject load(IocLoading iocLoading, String s) throws ObjectLoadException {
